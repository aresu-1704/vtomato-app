...
class LoginServices:
    ...
    def RegisterNewAccount(self, Email, PhoneNumber, Input_Password):
        try:
            ...
            password = hash_password.hash_password(Input_Password, salt)

            result = self._loginInfo.CreateNewAccount(Email=Email, PasswordHash=password, Salt=salt, PhoneNumber=PhoneNumber)
    ...
    def SendOTPToEmail(self, Email):
        result = self._loginInfo.FindAccountByEmail(Email)
